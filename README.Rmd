...
SciDataReportR now uses workflow-oriented canonical names for new code, while older public names remain available as compatibility aliases. For example, `CreatePCATable()` still works, but `CreatePCAObject()` better describes the reusable PCA object returned by that workflow.
