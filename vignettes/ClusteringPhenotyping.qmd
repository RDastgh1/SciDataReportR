...
df_Comparison <- df_Mclust$DataWithClusters %>%
  dplyr::select(.data$.row_id, .data$MclustCluster) %>%
  dplyr::left_join(
    df_KMeans$DataWithClusters %>%
      dplyr::select(.data$.row_id, .data$KMeansCluster),
    by = ".row_id")
...
Every successful full-pipeline 80% subsample refit records ARI for the
...
| `StabilityJaccard_Mean`, `StabilityJaccard_Min` | Mean cluster-level Jaccard recovery and the minimum cluster-specific mean recovery across successful 80% participant subsample refits [@jaccard1901]. |
