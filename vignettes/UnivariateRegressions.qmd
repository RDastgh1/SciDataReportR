---
title: "Screening predictors with MakeUnivariateRegressionTable()"
...
execute:
  ...
  message: false
vignette: >
  %\VignetteIndexEntry{Screening predictors with MakeUnivariateRegressionTable()}
...
Each outcome has its own column spanner. Effects are displayed as estimate
(95% CI), with significance stars appended and both the effect and p-value
bolded when significant.

## Detailed regression table

The larger table keeps the same wide outcome layout while exposing N, estimate,
standard error, confidence limits, and p-value separately.
