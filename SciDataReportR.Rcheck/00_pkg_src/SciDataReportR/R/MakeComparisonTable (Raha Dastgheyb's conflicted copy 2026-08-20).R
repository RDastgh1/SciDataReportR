...
#'
#' @section Choosing the options:
#' The defaults produce one test per variable, chosen from that variable's
#' type: a t-test for continuous variables, a chi-squared test for categorical
#' ones. The remaining arguments each answer a specific question the default
#' table cannot.
#'
#' **`AddEffectSize`** adds the standardized magnitude beside the p-value. A
#' p-value says whether a difference is detectable, not whether it is large,
#' and effect sizes are what make two significant rows comparable to each
#' other.
#'
#' **`AddPairwise`** matters as soon as there are more than two groups. The
#' omnibus test only says "these groups are not all the same"; it never says
#' which pair differs. Pairwise contrasts answer that, corrected across the
#' contrasts so that hunting through them does not inflate the error rate.
#' `PairwiseMethod` selects the correction - Bonferroni by default, `"fdr"`
#' when there are many contrasts, `"none"` for exploratory work. `Referent`
#' compares every group against one reference level instead of against each
#' other, which is usually what a control group is for.
#'
#' **`covariates`** replaces the simple test with a model-based one that holds
#' the named variables constant. An unadjusted group difference in a biomarker
#' may only reflect that the groups differ in age or sex; adjusting reports the
#' group difference that remains.
#'
#' **`Parametric = FALSE`** switches continuous comparisons to rank-based tests
#' (Wilcoxon, or Kruskal-Wallis for more than two groups), which is the right
#' choice for skewed measures or small, uneven groups.
#'
#' **`IncludeOverallN`** and **`IncludeMissing`** add an overall column and
#' explicit missing-value counts, for a table that has to stand on its own in a
#' manuscript.
#'
...
#' @param ShowNotes Whether to show the Analysis notes column. One of `"auto"`,
#' `"always"`, or `"never"`.
#' @param NotesPosition Analysis notes column position. One of `"last"`, `"after_test"`,
#' or `"before_pairwise"`.
...
#' )
#'
#' # Collapse the six APOE genotypes into three risk groups
...
#' )
#'
#' # Three groups, with pairwise contrasts
#' MakeComparisonTable(
#'   data = Labelled,
#'   group_var = "ApoeGroup",
#'   variables = vars_Compare,
...
#' )
#'
#' # FDR instead of Bonferroni across the contrasts
#' MakeComparisonTable(
#'   data = Labelled,
#'   group_var = "ApoeGroup",
...
#' )
#'
#' # Every group against one reference level
#' MakeComparisonTable(
#'   data = Labelled,
#'   group_var = "ApoeGroup",
#'   variables = c("age", "tau", "p_tau"),
...
#' )
#'
#' # Rank-based tests for the continuous variables
#' MakeComparisonTable(
#'   data = Labelled,
#'   group_var = "Diagnosis",
#'   variables = vars_Compare,
...
#' )
#'
#' # Overall column and explicit missing counts
#' MakeComparisonTable(
#'   data = Labelled,
#'   group_var = "Diagnosis",
#'   variables = vars_Compare,
#'   IncludeOverallN = TRUE,
#'   IncludeMissing = TRUE
#' )
#' }
#'
...
MakeComparisonTable <- function(data,
  ...
  group_label <- if (overall_mode) {
    NULL
  } else {
    ScidrDisplayLabels(DataFrame, CompVariable, Relabel)[[CompVariable]]
  }
  covariate_labels <- if (is.null(Covariates)) {
    character(0)
  } else {
    unname(ScidrDisplayLabels(DataFrame, Covariates, Relabel))
  }
      ...
      if (!is.null(Covariates)) {
        ...
        if (Parametric) {
          ...
          group_row <- if (is.null(a2)) {
            ...
            match(c(CompVariable, btick(CompVariable)), rownames(a2), nomatch = 0L) %>%
              unique() %>%
              purrr::keep(~ .x > 0L) %>%
              dplyr::first(default = NA_integer_)
          }
  ...
  if (AddPairwise && length(combos_all) > 0) {
    ...
    if (nrow(pw_long) > 0) {
      ...
      for (col in setdiff(names(pw_wide), "variable")) {
        ...
        # Built as `.data$pw_1` rather than `.data[["pw_1"]]`. Both select the
        # same column, but gtsummary::tbl_merge() rewrites column references
        # only in the `$` form when it suffixes names for merged tables. With
        # the `[[` form the styling rule keeps pointing at the pre-merge name,
        # and the merged table fails to print with "Column `pw_1` not found".
        # This is what MakeFacetCatComparisonTable() relies on.
        col_pronoun <- rlang::call2("$", rlang::sym(".data"), rlang::sym(col))
        rows_significant <- rlang::expr(
          .data$.is_main_row & !is.na(!!col_pronoun) & !!col_pronoun <= 0.05
        )
  ...
  if ("Notes" %in% names(tbl$table_body)) {
    ...
    if (ShowNotes == "never" || (ShowNotes == "auto" && !has_notes)) {
      ...
      tbl <- tbl %>%
        gtsummary::modify_header(Notes ~ "**Analysis notes**") %>%
  ...
  categorical_method <- switch(
    CatMethod,
    auto = "chi-square or Fisher's exact test, selected from expected counts",
    chisq = "Pearson chi-square test",
    fisher = "Fisher's exact test"
  )

  continuous_method <- if (is.null(Covariates)) {
    if (Parametric) {
      "Welch t-test (two groups) or ANOVA (more than two groups)"
    } else {
      "Wilcoxon rank-sum test (two groups) or Kruskal-Wallis test (more than two groups)"
    }
  } else if (Parametric) {
    "Type II ANCOVA"
  } else {
    "HC3 robust Type II ANCOVA"
  }

  categorical_method <- if (is.null(Covariates)) {
    categorical_method
  } else if (MultiCatAdjusted == "none") {
    "logistic likelihood-ratio test for binary outcomes; multicategory outcomes are not adjusted"
  } else {
    "logistic likelihood-ratio test for binary outcomes or multinomial likelihood-ratio test for multicategory outcomes"
  }

  methods_note <- paste0(
    "p-values use ", continuous_method,
    " for continuous outcomes and ", categorical_method,
    " for categorical outcomes."
  )

  if (AddPairwise) {
    pairwise_phrase <- if (PairwiseMethod == "none") {
      "unadjusted"
    } else {
      paste0(PairwiseMethod, "-adjusted")
    }
    methods_note <- paste0(
      methods_note,
      " Pairwise p-values are ", pairwise_phrase, " within outcome."
    )
  }

  cap <- sprintf(
    "Comparison by %s (values: %s).",
    group_label,
    if (ParametricDisplay) "mean (SD)" else "median [IQR]"
  )

  if (length(covariate_labels)) {
    cap <- paste0(
      cap,
      " p-values adjusted for ",
      paste(covariate_labels, collapse = ", "),
      "."
    )
  }
