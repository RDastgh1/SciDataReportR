...
.PrepareClusterNumeric <- function(data, variables, Scaling) {
  ...
  df_scidr <- data
  if (!".scidr_rowid" %in% names(df_scidr)) df_scidr$.scidr_rowid <- seq_len(nrow(df_scidr))
...
.ClusterBootstrapStability <- function(data, reference, fit_project,
  ...
  for (replicate in seq_len(resamples)) {
    ...
    sampled <- sample(valid_reference, length(valid_reference), replace = TRUE)
  ...
  list(settings = list(resamples = resamples, seed = seed,
    refit_scope = "full_pipeline"), replicates = replicates,
    cluster_recovery = cluster_recovery, summary = summary,
    failures = dplyr::filter(replicates, .data$Status != "success"))
}

.ClusterOutput <- function(prep, cluster, ClusterName, individual, ModelInfo,
    Stability = NULL) {
  cluster_full <- rep(NA_integer_, nrow(prep$data))
  cluster_full[prep$complete_rows] <- as.integer(cluster)
  df_out <- prep$data
  if (ClusterName %in% names(df_out)) message("Overwriting existing column '", ClusterName, "'.")
  df_out[[ClusterName]] <- cluster_full
  individual <- dplyr::mutate(individual, .scidr_rowid = prep$data$.scidr_rowid, RowID = prep$data$.scidr_rowid,
                              Cluster = cluster_full)
  list(df_with_clusters = df_out, ProbFit = list(individual = individual),
...
# stores in ModelInfo_SOM$SOMFit and $PhenotypeReference.
...
# the same way ProjectSOMCluster() does.
...
.MclustCandidate <- function(X, k, model_name) {
  ...
  list(fit = fit, error = NA_character_, row = dplyr::tibble(
    Model = model_name, Classes = as.integer(fit$G), BIC = fit$bic,
...
#' @param final_k,final_model Finalized cluster count and covariance model.
#' @param Scaling Frozen preprocessing: `"Center and Scale"`, `"Center Only"`,
#'   `"Scale Only"`, `"None"`, or `"PreZScored"`.
#' @param ClusterName Output cluster column name.
...
#'   holds bootstrap agreement and per-cluster recovery.
#' @examples
#' \donttest{
...
#' review <- CreateMclustClusterModel(
#'   df_Training, vars_Numeric, k_range = 2:4, model_names = "EEI",
...
#' review$Stability$plots$bootstrap_agreement
...
#' model$ModelInfo$FitDiagnostics$plots$distance_by_cluster_box
#' model$ProbFit$plots$confidence_density
#' projected <- ProjectMclustCluster(model, df_Projection)
#' projected$ProjectionFit$plots$projection_fit_class_bar
#' }
#' @export
CreateMclustClusterModel <- function(data, variables = NULL,
    method = c("exploratory", "finalize"), k_range = 2:10,
    model_names = c("EEI", "VVI", "EEE"), final_k = NULL, final_model = NULL,
    Scaling = c("Center and Scale", "Center Only", "Scale Only", "None", "PreZScored"),
    ClusterName = "Cluster", seed = 93421L, stability_resamples = 0L,
  ...
  method <- .ClusterMethod(method); Scaling <- match.arg(Scaling)
  ...
  if (method == "finalize") {
    ...
    k_range <- final_k; model_names <- final_model
  }
  set.seed(seed)
  candidates <- unlist(lapply(model_names, function(m) lapply(k_range, function(k) .MclustCandidate(X, k, m))), recursive = FALSE)
  ...
  if (stability_resamples > 0L) {
    stabilities <- lapply(seq_along(good), function(i) {
      candidate <- good[[i]]; k <- candidate$row$Classes[[1]]; model_name <- candidate$row$Model[[1]]
      ...
      .ClusterBootstrapStability(prep$data, reference, function(boot, original) {
        fitted <- CreateMclustClusterModel(boot, prep$variables, method = "finalize",
          final_k = k, final_model = model_name, Scaling = Scaling, seed = seed,
          stability_resamples = 0L)
        ProjectMclustCluster(fitted, original)$ProbFit$individual$Cluster
      }, stability_resamples, stability_seed + i, list(Model = model_name, Classes = k),
  ...
  ModelInfo <- list(
    ...
    final_k = best$G, final_model = best$modelName)
  base <- .ClusterOutput(prep, best$classification, ClusterName, individual,
    ModelInfo = ModelInfo, Stability = Stability)
  ...
  ModelInfo$plots <- .DropNullPlots(list(
    ...
    profiles = PlotClusterProfiles(
      base$df_with_clusters, ClusterName, prep$variables)))
  ...
  out <- c(list(method = method, vars_used = prep$variables, ClusterName = ClusterName,
                complete_rows = prep$complete_rows,
                Preprocessing = list(Scaling = Scaling, ZScoreObject = prep$ZScoreObject),
                fit_plot = PlotClusterFitReview(fit_table)), base)
  out$Stability <- Stability
  out$ModelInfo_Mclust <- out$ModelInfo
  out$CandidateAudit <- dplyr::mutate(fit_table, Status = "success",
  ...
  out$Specification <- .ClusterSpecification(
    ...
    list(k = best$G, model = best$modelName), prep$complete_rows,
...
#' @param object A `CreateMclustClusterModel()` result.
#' @param new_df New observations.
#' @param ClusterName Optional output cluster column name.
...
#'   training-versus-projected occupancy comparison, and the fit-class bar;
...
#' }
#' @export
ProjectMclustCluster <- function(object, new_df, ClusterName = object$ClusterName) {
  if (!inherits(object, "Pipeline_Mclust")) stop("object must be a Pipeline_Mclust result.")
  ...
  out <- c(list(vars_used = object$vars_used, ClusterName = ClusterName, complete_rows = prep$complete_rows,
...
#'   size) and bootstrap stability. Figures sit beside what they describe:
...
#' df_Projection <- dplyr::filter(SimulatedPhenotypeData, .data$Cohort == "Projection")
#' vars_Numeric <- paste0("Var", 1:12)
#' review <- CreateKMeansClusterModel(
...
#' model$ModelInfo$plots$centre_heatmap
#' model$ModelInfo$plots$separation_map
#' model$ModelInfo$FitDiagnostics$plots$distance_ecdf
#' projected <- ProjectKMeansCluster(model, df_Projection)
#' projected$ProjectionFit$plots$projection_fit_class_bar
#' }
#' @export
CreateKMeansClusterModel <- function(data, variables = NULL,
    method = c("exploratory", "finalize"), k_range = 2:10, final_k = NULL,
    Scaling = c("Center and Scale", "Center Only", "Scale Only", "None", "PreZScored"),
    ClusterName = "Cluster", seed = 93421L, nstart = 50L,
  ...
  if (stability_resamples > 0L) {
    stabilities <- lapply(seq_along(ks), function(i) {
      ...
      .ClusterBootstrapStability(prep$data, reference, function(boot, original) {
        fitted <- CreateKMeansClusterModel(boot, prep$variables, method = "finalize",
          final_k = k, Scaling = Scaling, seed = seed, nstart = nstart,
          stability_resamples = 0L)
        ProjectKMeansCluster(fitted, original)$ProbFit$individual$Cluster
  ...
  base <- .ClusterOutput(prep, best$cluster, ClusterName, ind,
  ...
  if (!is.null(Stability)) Stability$plots <- .ClusterStabilityPlots(Stability)
  out <- c(list(method = method, vars_used = prep$variables, ClusterName = ClusterName, complete_rows = prep$complete_rows,
                Preprocessing = list(Scaling = Scaling, ZScoreObject = prep$ZScoreObject),
...
#' }
#' @export
ProjectKMeansCluster <- function(object, new_df, ClusterName = object$ClusterName) {
  if (!inherits(object, "Pipeline_KMeans")) stop("object must be a Pipeline_KMeans result.")
  ...
  ProjectionFit$plots$separation_map <- .ClusterSeparationMap(
    ...
    prefix = "Projected")
  out <- c(list(vars_used = object$vars_used, ClusterName = ClusterName, complete_rows = prep$complete_rows, Preprocessing = object$Preprocessing), base)
  out$ProbFit <- .ClusterProbFit(
    individual, "AssignmentMargin",
...
.CreatePCACluster <- function(data, variables, algorithm, method, k_range,
    final_k, final_model, Scaling, ClusterName, seed, pca_variance_threshold,
    nstart = NULL, model_names = c("EEI", "VVI", "EEE"),
    stability_resamples = 0L, stability_seed = seed + 1L,
    stability_progress = FALSE) {
  stability_resamples <- .ValidateClusterStability(
    stability_resamples, stability_seed, stability_progress)
  ...
  FitScoreModel <- function(fit_method, candidate_k = final_k,
    ...
    if (algorithm == "mclust") {
      CreateMclustClusterModel(
        ...
        model_names = model_names, final_k = candidate_k,
        final_model = candidate_model, Scaling = "None",
        ClusterName = ClusterName, seed = seed, stability_resamples = 0L)
    } else {
      CreateKMeansClusterModel(
        df_Scores, score_vars, method = fit_method, k_range = k_range,
        final_k = candidate_k, Scaling = "None", ClusterName = ClusterName,
  ...
  if (stability_resamples > 0L) {
    ...
    stabilities <- lapply(seq_len(nrow(candidate_rows)), function(i) {
      ...
      reference_fit <- .CreatePCACluster(
        ...
        candidate_k, candidate_model, Scaling, ClusterName, seed,
        pca_variance_threshold, nstart, model_names, 0L, stability_seed, FALSE)
      ...
      .ClusterBootstrapStability(data, reference, function(boot, original) {
        fitted <- .CreatePCACluster(
          ...
          candidate_k, candidate_model, Scaling, ClusterName, seed,
          pca_variance_threshold, nstart, model_names, 0L,
          stability_seed, FALSE)
        .ProjectPCACluster(fitted, original, algorithm, ClusterName)$ProbFit$individual$Cluster
  ...
  indicator_names <- setdiff(names(reduced_individual), c(".scidr_rowid", "RowID", "Cluster"))
  ...
  base <- .ClusterOutput(
    prep, reduced_individual$Cluster, ClusterName, individual,
  ...
  if (!is.null(Stability)) Stability$plots <- .ClusterStabilityPlots(Stability)
  out <- c(list(
    method = method, vars_used = prep$variables, ClusterName = ClusterName,
    complete_rows = prep$complete_rows,
    Preprocessing = list(
      Scaling = Scaling, ZScoreObject = prep$ZScoreObject, PCAObject = pca,
  ...
  out$ModelInfo_PCA <- list(PCAObject = pca, Scaling = Scaling,
    ZScoreObject = prep$ZScoreObject, PCVariables = score_vars)
  out$CandidateAudit <- dplyr::mutate(review$fit_table, Status = "success",
    RuntimeSeconds = NA_real_, Warnings = NA_character_, Error = NA_character_)
  out$Specification <- .ClusterSpecification(
  ...
  class(out) <- c(
    if (algorithm == "mclust") "Pipeline_PCAMclust" else "Pipeline_PCAKMeans",
    class(selected))
  out
}
...
.ProjectPCACluster <- function(object, new_df, algorithm, ClusterName) {
  prep <- .ProjectClusterNumeric(object, new_df)
  ...
  base$Preprocessing <- list(Scaling = "None", ZScoreObject = NULL)
  class(base) <- if (algorithm == "mclust") "Pipeline_Mclust" else "Pipeline_KMeans"
  projected_scores <- if (algorithm == "mclust") {
    ProjectMclustCluster(base, pca_projection$Scores, ClusterName)
  } else ProjectKMeansCluster(base, pca_projection$Scores, ClusterName)
  reduced <- projected_scores$ProbFit$individual
  indicator_names <- setdiff(names(reduced), c(".scidr_rowid", "RowID", "Cluster"))
  ...
  out <- c(list(
    vars_used = object$vars_used, ClusterName = ClusterName,
    complete_rows = complete, Preprocessing = object$Preprocessing), result)
  out$ProbFit <- projected_scores$ProbFit
  out$ProbFit$individual <- individual
  ProjectionFit <- projected_scores$ProjectionFit
  out$ProjectionFit <- ProjectionFit
  out$df_with_clusters$Projection_Fit_Class <- individual$Projection_Fit_Class
  class(out) <- c(
...
#'   `ModelInfo$fit_table` and `ModelInfo$AHP`.
#' @examples
#' \donttest{
#' data("SimulatedPhenotypeData")
#' df_Training <- dplyr::filter(SimulatedPhenotypeData, .data$Cohort == "Training")
#' vars_Numeric <- paste0("Var", 1:12)
#' review <- CreatePCAMclustClusterModel(
#'   df_Training, vars_Numeric, k_range = 2:4, model_names = "EEI",
#'   stability_resamples = 2
#' )
#' review$ModelInfo$fit_table
...
#' }
#' @export
CreatePCAMclustClusterModel <- function(data, variables = NULL, method = c("exploratory", "finalize"),
    k_range = 2:10, model_names = c("EEI", "VVI", "EEE"), final_k = NULL, final_model = NULL,
    Scaling = c("Center and Scale", "Center Only", "Scale Only", "None", "PreZScored"),
    ClusterName = "Cluster", seed = 93421L, pca_variance_threshold = 0.85,
  ...
  .CreatePCACluster(data, variables, "mclust", .ClusterMethod(method), k_range, final_k, final_model,
                    match.arg(Scaling), ClusterName, seed, pca_variance_threshold, model_names = model_names,
...
#' df_Training <- dplyr::filter(SimulatedPhenotypeData, .data$Cohort == "Training")
#' df_Projection <- dplyr::filter(SimulatedPhenotypeData, .data$Cohort == "Projection")
#' model <- CreatePCAMclustClusterModel(
#'   df_Training, paste0("Var", 1:12), method = "finalize",
#'   final_k = 4, final_model = "EEI", stability_resamples = 2
#' )
#' projected <- ProjectPCAMclustCluster(model, df_Projection)
#' head(projected$ProbFit$individual)
#' projected$ProjectionFit$overall_fit_summary
#' projected$ProjectionFit$plots$projection_fit_class_bar
#' }
#' @export
ProjectPCAMclustCluster <- function(object, new_df, ClusterName = object$ClusterName) {
  if (!inherits(object, "Pipeline_PCAMclust")) stop("object must be a Pipeline_PCAMclust result.")
  .ProjectPCACluster(object, new_df, "mclust", ClusterName)
}
...
#' @inheritParams CreateKMeansClusterModel
...
#'   restated in the original measurement scale.
#' @examples
#' \donttest{
#' data("SimulatedPhenotypeData")
#' df_Training <- dplyr::filter(SimulatedPhenotypeData, .data$Cohort == "Training")
#' vars_Numeric <- paste0("Var", 1:12)
#' review <- CreatePCAKMeansClusterModel(
#'   df_Training, vars_Numeric, k_range = 2:5, nstart = 10,
#'   stability_resamples = 2
#' )
#' review$ModelInfo$fit_table
...
#' }
#' @export
CreatePCAKMeansClusterModel <- function(data, variables = NULL, method = c("exploratory", "finalize"),
    k_range = 2:10, final_k = NULL,
    Scaling = c("Center and Scale", "Center Only", "Scale Only", "None", "PreZScored"),
    ClusterName = "Cluster", seed = 93421L, nstart = 50L, pca_variance_threshold = 0.85,
    stability_resamples = 0L, stability_seed = seed + 1L, stability_progress = FALSE) {
  .CreatePCACluster(data, variables, "kmeans", .ClusterMethod(method), k_range, final_k, NULL,
                    match.arg(Scaling), ClusterName, seed, pca_variance_threshold, nstart,
                    stability_resamples = stability_resamples, stability_seed = stability_seed,
                    stability_progress = stability_progress)
}

#' Project data onto a PCA + K-means model
#' @inheritParams ProjectKMeansCluster
...
#'   final_k = 4, nstart = 10, stability_resamples = 2
#' )
#' projected <- ProjectPCAKMeansCluster(model, df_Projection)
#' head(projected$ProbFit$individual)
#' projected$ProjectionFit$plots$projection_fit_class_bar
#' }
#' @export
ProjectPCAKMeansCluster <- function(object, new_df, ClusterName = object$ClusterName) {
  if (!inherits(object, "Pipeline_PCAKMeans")) stop("object must be a Pipeline_PCAKMeans result.")
  .ProjectPCACluster(object, new_df, "kmeans", ClusterName)
}
...
#' @param minPts Deprecated-compatible shortcut for a finalized `minPts` fit.
...
#'   bootstrap ARI/Jaccard and noise-recovery metrics. Figures sit beside what
...
#' df_Training <- dplyr::filter(SimulatedPhenotypeData, .data$Cohort == "Training")
#' df_Projection <- dplyr::filter(SimulatedPhenotypeData, .data$Cohort == "Projection")
#' review <- CreateHDBSCANClusterModel(
...
CreateHDBSCANClusterModel <- function(data, variables = NULL,
  ...
  stability_resamples <- .ValidateClusterStability(
    stability_resamples, stability_seed, stability_progress)
  if (!is.null(minPts)) { method <- "finalize"; final_minPts <- minPts; if (is.null(final_cluster_selection_epsilon)) final_cluster_selection_epsilon <- 0 }
  method <- .ClusterMethod(method); Scaling <- match.arg(Scaling); prep <- .PrepareClusterNumeric(data, variables, Scaling)
  X <- prep$X[prep$complete_rows, , drop = FALSE]; if (!nrow(X)) stop("No complete rows available for clustering.")
  ...
  if (stability_resamples > 0L) {
    stabilities <- lapply(seq_len(nrow(grid)), function(i) {
      ...
      .ClusterBootstrapStability(prep$data, reference, function(boot, original) {
        fitted <- CreateHDBSCANClusterModel(boot, prep$variables, method = "finalize",
          final_minPts = grid$MinPts[[i]], final_cluster_selection_epsilon = grid$Epsilon[[i]],
          Scaling = Scaling, seed = seed, stability_resamples = 0L)
        ProjectHDBSCANCluster(fitted, original)$ProbFit$individual$Cluster
      }, stability_resamples, stability_seed + i,
  ...
  base <- .ClusterOutput(prep, fit$cluster, ClusterName, ind,
    ModelInfo, Stability = Stability)
  individual <- base$ProbFit$individual
  ModelInfo$FitDiagnostics <- .ClusterFitDiagnostics(
  ...
  ModelInfo$plots <- .DropNullPlots(list(
    ...
    profiles = PlotClusterProfiles(
      dplyr::filter(base$df_with_clusters, .data[[ClusterName]] > 0),
      ClusterName, prep$variables)))
  base$ModelInfo <- ModelInfo
  base$ProbFit <- .ClusterProbFit(
...
#'   training-versus-projected occupancy comparison, the fit-class bar, and the
...
#' }
#' @export
ProjectHDBSCANCluster <- function(object, new_df, ClusterName = object$ClusterName) {
  if (!inherits(object, "Pipeline_HDBSCAN")) stop("object must be a Pipeline_HDBSCAN result.")
...
#' @param k Backward-compatible shortcut for a finalized medoid count.
...
#'   and bootstrap stability. Figures sit beside what they describe:
...
#' review <- CreateGowerPAMClusterModel(
...
#' review$fit_plot
#' review$ModelInfo$plots$silhouette_by_k
#' model <- CreateGowerPAMClusterModel(
...
#' projected <- ProjectGowerPAMCluster(model, df_Projection)
#' projected$ProjectionFit$plots$projection_fit_class_bar
#' }
#' @export
CreateGowerPAMClusterModel <- function(data, variables = NULL,
    method = c("exploratory", "finalize"), k_range = 2:10, final_k = NULL,
    k = NULL, ClusterName = "Cluster", seed = 93421L, stability_resamples = 0L,
    stability_seed = seed + 1L, stability_progress = FALSE) {
  ...
  if (!is.null(k)) { method <- "finalize"; final_k <- k }
  ...
  if (stability_resamples > 0L) {
    stabilities <- lapply(seq_along(ks), function(i) {
      ...
      .ClusterBootstrapStability(df_scidr, reference, function(boot, original) {
        fitted <- CreateGowerPAMClusterModel(boot, variables, method = "finalize",
          final_k = ks[[i]], seed = seed, stability_resamples = 0L)
        ProjectGowerPAMCluster(fitted, original)$ProbFit$individual$Cluster
  ...
  gower_assignment[[ClusterName]] <- fit$clustering
  p_gower_map <- PlotClusterMap(
    gower_assignment, "GowerDimension1", "GowerDimension2", ClusterName,
  ...
  ModelInfo <- list(
    ...
    final_k = k)
  base <- .ClusterOutput(prep, fit$clustering, ClusterName, ind,
    ModelInfo, Stability = Stability)
  individual <- base$ProbFit$individual
  ModelInfo$FitDiagnostics <- .ClusterFitDiagnostics(
  ...
  ModelInfo$plots <- .DropNullPlots(list(
    ...
    profiles = if (length(numeric_variables)) {
      PlotClusterProfiles(base$df_with_clusters, ClusterName, numeric_variables)
    } else p_categorical))
  base$ModelInfo <- ModelInfo
  base$ProbFit <- .ClusterProbFit(
    individual, "AssignmentMargin",
  ...
  if (!is.null(Stability)) Stability$plots <- .ClusterStabilityPlots(Stability)
  out <- c(list(method = method, vars_used = variables, ClusterName = ClusterName, complete_rows = complete, Preprocessing = list(Scaling = "Gower", ZScoreObject = NULL), fit_plot = PlotClusterFitReview(rows)), base)
  out$Stability <- Stability
  ...
  class(out) <- c("Pipeline_GowerPAM", class(out)); out
}
...
#'   distances, the training-versus-projected occupancy comparison, and the
#'   fit-class bar.
#' @examples
#' \donttest{
#' data("SimulatedPhenotypeData")
...
#' }
#' @export
ProjectGowerPAMCluster <- function(object, new_df, ClusterName = object$ClusterName) {
  if (!inherits(object, "Pipeline_GowerPAM")) stop("object must be a Pipeline_GowerPAM result.")
  ...
  prep <- list(data = new_df_supported, complete_rows = stats::complete.cases(new_df_supported[object$vars_used]),
               variables = object$vars_used); if (!".scidr_rowid" %in% names(prep$data)) prep$data$.scidr_rowid <- seq_len(nrow(prep$data))
