...
#'
#' Bootstrap stability is assessed by refitting the full SOM/LPA pipeline.
...
CreateClusterModel_SOM_MClust <- function(data,
    ...
    if (stability_resamples > 0) {
      ...
      Stability <- list(
        settings = list(
          ...
          seed = stability_seed,
          refit_scope = "full_pipeline", coassignment_limit = 2000L,
  ...
  mdata_cols <- intersect(
    c("AIC", "BIC", "Entropy", "BLRTPValue", "ReproducibilityScore"),
    names(mdata_wide)
  )
  ...
  if ("BLRTPValue" %in% mdata_cols) {
    fit_plot <- fit_plot + ggplot2::geom_hline(
      data = dplyr::tibble(name = "BLRTPValue", value = 0.05),
