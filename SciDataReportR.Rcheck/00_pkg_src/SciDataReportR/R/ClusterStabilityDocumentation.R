...
#'
#' @name cluster-stability-output
#' @title Interpret stability output from a finalized clustering model
#' @description This shared section is included in the reference documentation
#'   for every finalized clustering constructor. `Stability` is `NULL` when
...
#' participants are selected once, all preprocessing and any reduction (PCA,
#' MCA, or SOM) are refit, the
#' selected clustering method is refit, and the original complete training
#' participants are projected into that subsample fit for comparison with the
#' original fitted partition. It is an internal sensitivity analysis, not
#' independent-cohort validation.
#'
#' `Stability$settings` records the analysis provenance:
#' \itemize{
#'   \item `resamples`: requested number of 80% subsample refits.
#'   \item `seed`: seed used to select subsamples.
#'   \item `refit_scope`: always `"full_pipeline"`, meaning preprocessing,
...
#'   calculated.
#'   \item `noise_policy`: whether the method has noise labels. For ordinary
#'   methods it is `"all clusters included"`; HDBSCAN variants retain noise in
#'   global partition metrics but exclude it from per-cluster inclusion and
#'   co-assignment summaries.
#' }
#'
...
#' \itemize{
#'   \item `ARI`: adjusted Rand index, agreement corrected for chance; higher
#'   is better and can be negative when agreement is worse than chance.
#'   \item `VI`: variation of information, the information lost or gained when
#'   changing partitions; lower is better and zero is identical.
#'   \item `NMI`: normalized mutual information; higher is better and one is
#'   identical.
#'   \item `FowlkesMallows`: pairwise clustering agreement; higher is better
...
#' phenotype.
#'
#' Metric sources: Hubert and Arabie (1985) define ARI; Jaccard (1901) defines
#' the overlap coefficient; Meila (2005) defines VI; Strehl and Ghosh (2002)
#' describe NMI for partition comparison; Fowlkes and Mallows (1983) define
#' their pairwise index; and Monti et al. (2003) describe resampling-based
#' consensus co-assignment.
#'
#' @references Hubert L, Arabie P. Comparing partitions. *Journal of
#' Classification*. 1985;2:193-218. Jaccard P. Nouvelles recherches sur la
#' distribution florale. *Bulletin de la Societe Vaudoise des Sciences
#' Naturelles*. 1901;37:547-579. Meila M. Comparing clusterings: an axiomatic
#' view. *Proceedings of ICML*. 2005:577-584. Strehl A, Ghosh J. Cluster
#' ensembles---a knowledge reuse framework for combining multiple partitions.
#' *Journal of Machine Learning Research*. 2002;3:583-617. Fowlkes EB,
#' Mallows CL. A method for comparing two hierarchical clusterings. *Journal
#' of the American Statistical Association*. 1983;78:553-569. Monti S,
#' Tamayo P, Mesirov J, Golub T. Consensus clustering: a resampling-based
#' method for class discovery and visualization of gene expression microarray
#' data. *Machine Learning*. 2003;52:91-118.
#' @keywords internal
NULL
