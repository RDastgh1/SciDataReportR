...
- 80% participant subsample stability refits the **entire pipeline**, including scaling and
...
PrintFitMetrics <- function(object, metrics) {
  object$ModelInfo$fit_table %>%
    dplyr::select(dplyr::any_of(c(
      "Model", "ModelName", "Classes", "MinPts", "Epsilon", metrics,
...
df_MclustReview <- CreateClusterModel_MClust(
  df_Training, vars_Continuous, k_range = 2:10, models = 1,
...
df_Mclust <- CreateClusterModel_MClust(
  ...
  final_model = 1, ClusterVariableName = "MclustCluster",
  stability_resamples = 2
)
df_MclustProjected <- ProjectCluster(df_Mclust, df_Projection)
PrintPlots(
  df_Mclust$ModelInfo$plots,
  c("centre_heatmap", "centre_profile", "profiles"))
...
df_KMeans <- CreateClusterModel_KMeans(
  df_Training, vars_Continuous, method = "finalize", final_k = 4,
  nstart = 20, ClusterVariableName = "KMeansCluster", stability_resamples = 2
)
df_KMeansProjected <- ProjectCluster(df_KMeans, df_Projection)
PrintPlots(
  df_KMeans$ModelInfo$plots,
  c("silhouette", "centre_heatmap", "profiles"))
...
80% subsampling, then use the frozen training transformation for projection.
...
df_PCAMclust <- CreateClusterModel_PCA_MClust(
  df_Training, vars_Continuous, method = "finalize", final_k = 4,
  final_model = 1, ClusterVariableName = "PCAMclustCluster",
  stability_resamples = 2
)
df_PCAMclustProjected <- ProjectCluster(df_PCAMclust, df_Projection)
PrintPlots(
  df_PCAMclust$ModelInfo$plots,
  c("scree", "loadings", "profiles"))
...
df_PCAKMeans <- CreateClusterModel_PCA_KMeans(
  df_Training, vars_Continuous, method = "finalize", final_k = 4,
  nstart = 20, ClusterVariableName = "PCAKMeansCluster",
  stability_resamples = 2
)
df_PCAKMeansProjected <- ProjectCluster(df_PCAKMeans, df_Projection)
PrintPlots(
  df_PCAKMeans$ModelInfo$plots,
  c("scree", "silhouette", "profiles"))
...
df_HDBSCAN <- CreateClusterModel_HDBSCAN(
  ...
  final_cluster_selection_epsilon = 0, ClusterVariableName = "HDBSCANCluster",
  stability_resamples = 2
)
df_HDBSCANProjected <- ProjectCluster(df_HDBSCAN, df_Projection)
PrintPlots(
  df_HDBSCAN$ModelInfo$plots,
  c("density_review", "cluster_persistence", "profiles"))
...
resamples only to keep build time reasonable.
...
vars_Clust <- paste0("Var", 1:12)
Obj_ClustReview <- CreateClusterModel_SOM_MClust(
  df_Training, vars_Clust, method = "exploratory", k_range = 2:10,
  models = c(1, 3), som_xdim = 5, som_ydim = 5, stability_resamples = 2,
  Relabel = FALSE, min_nodes_per_cluster = NULL
)
Obj_ClustReview$ModelInfo_MClust$fit_table
Obj_ClustReview$ModelInfo_MClust$AHP$recommendation
Obj_ClustReview$Stability$summary
print(Obj_ClustReview$fit_plot)
Obj_ClustReview$ModelInfo_SOM$plots$Circular
Obj_ClustReview$ModelInfo_SOM$plots$Line
Obj_ClustReview$ModelInfo_SOM$plots$Cloud
```

```{r}
#| label: SOMFinalize
Obj_ClustFinal <- CreateClusterModel_SOM_MClust(
  df_Training, vars_Clust, method = "finalize", final_k = 4,
  final_model = 3, som_xdim = 5, som_ydim = 5,
  stability_resamples = 2,
  Relabel = FALSE, min_nodes_per_cluster = NULL
)
ObjClustProjected <- ProjectCluster(Obj_ClustFinal, df_Projection)
PrintPlots(Obj_ClustFinal$ModelInfo_SOM$plots)
PrintPlots(Obj_ClustFinal$ModelInfo_SOM$SOMFit$plots)
PrintPlots(Obj_ClustFinal$ModelInfo_SOM$ProjectionReference$plots)
PrintPlots(Obj_ClustFinal$ProbFit$plots)
PrintPlots(Obj_ClustFinal$Stability$plots)
PrintPlots(
  ObjClustProjected$ProjectionFit$plots,
  c("projection_fit_class_bar", "som_grid_map"))
```
...
df_Gower <- CreateClusterModel_Gower_PAM(
  ...
  ClusterVariableName = "GowerCluster", stability_resamples = 2
)
df_GowerProjected <- ProjectCluster(df_Gower, df_Projection)
PrintPlots(
...
df_LCA <- CreateClusterModel_LatentClass(
  ...
  nrep = 5, ClusterVariableName = "LCACluster", stability_resamples = 2
)
df_LCAProjected <- ProjectCluster(df_LCA, df_Projection)
PrintPlots(
...
[@greenacre2017]. Each 80% subsample refits MCA before refitting the mixture.
...
df_MCAMclustReview <- CreateClusterModel_MCA_MClust(
  df_Training, vars_Categorical, k_range = 2:10, models = 1,
  stability_resamples = 2
)
PrintFitMetrics(
  df_MCAMclustReview,
...
df_MCAMclust <- CreateClusterModel_MCA_MClust(
  df_Training, vars_Categorical, method = "finalize", final_k = 4,
  final_model = 1, ClusterVariableName = "MCAMclustCluster",
  stability_resamples = 2
)
df_MCAMclustProjected <- ProjectCluster(df_MCAMclust, df_Projection)
PrintPlots(
  df_MCAMclust$ModelInfo$plots,
  c("scree", "categorical_composition"))
...
df_TruthAgreement <- dplyr::tibble(
  ...
  TrainingARI = c(
    CalculateTruthARI(df_Training$TruthCluster, df_Mclust$DataWithClusters$MclustCluster),
    CalculateTruthARI(df_Training$TruthCluster, df_KMeans$DataWithClusters$KMeansCluster),
    CalculateTruthARI(df_Training$TruthCluster, df_PCAMclust$DataWithClusters$PCAMclustCluster),
    CalculateTruthARI(df_Training$TruthCluster, df_PCAKMeans$DataWithClusters$PCAKMeansCluster),
    CalculateTruthARI(df_Training$TruthCluster, df_Gower$DataWithClusters$GowerCluster),
    CalculateTruthARI(df_Training$TruthCluster, df_LCA$DataWithClusters$LCACluster),
    CalculateTruthARI(df_Training$TruthCluster, df_MCAMclust$DataWithClusters$MCAMclustCluster),
    CalculateTruthARI(df_Training$TruthCluster, Obj_ClustFinal$DataWithClusters$Cluster)),
  ProjectionARI = c(
    CalculateTruthARI(df_Projection$TruthCluster, df_MclustProjected$ProbFit$individual$Cluster),
    CalculateTruthARI(df_Projection$TruthCluster, df_KMeansProjected$ProbFit$individual$Cluster),
    CalculateTruthARI(df_Projection$TruthCluster, df_PCAMclustProjected$ProbFit$individual$Cluster),
    CalculateTruthARI(df_Projection$TruthCluster, df_PCAKMeansProjected$ProbFit$individual$Cluster),
    CalculateTruthARI(df_Projection$TruthCluster, df_GowerProjected$ProbFit$individual$Cluster),
    CalculateTruthARI(df_Projection$TruthCluster, df_LCAProjected$ProbFit$individual$Cluster),
    CalculateTruthARI(df_Projection$TruthCluster, df_MCAMclustProjected$ProbFit$individual$Cluster),
    CalculateTruthARI(df_Projection$TruthCluster, ObjClustProjected$ProbFit$individual$Cluster))
)
df_TruthAgreement
```
...
df_DensityTruth <- dplyr::tibble(
  ...
  ARI = c(
    CalculateTruthARI(
      ...
      df_HDBSCAN$DataWithClusters$HDBSCANCluster),
    CalculateTruthARI(
      df_Projection$TruthDensityGroup,
      df_HDBSCANProjected$ProbFit$individual$Cluster)),
  NoiseProportion = c(
    mean(df_HDBSCAN$DataWithClusters$HDBSCANCluster == 0, na.rm = TRUE),
...
df_Comparison <- df_Mclust$DataWithClusters %>%
  dplyr::mutate(RowIndex = dplyr::row_number()) %>%
  dplyr::select(.data$RowIndex, .data$MclustCluster) %>%
  dplyr::left_join(
    df_KMeans$DataWithClusters %>%
      dplyr::mutate(RowIndex = dplyr::row_number()) %>%
      dplyr::select(.data$RowIndex, .data$KMeansCluster),
    by = "RowIndex")
...
PlotClusterBoxplot(
  data = df_Mclust$DataWithClusters,
...
Every successful full-pipeline 80% subsample refit also records variation of
...
| `StabilityARI_Mean`, `StabilityARI_P05` | Mean and fifth percentile of the adjusted Rand index across successful full-pipeline 80% participant subsample refits without replacement [@hubert1985]. ARI can be negative when agreement is worse than chance. |
| `StabilityJaccard_Mean`, `StabilityJaccard_Min` | Mean and minimum label-matched per-cluster Jaccard recovery across successful 80% participant subsample refits [@jaccard1901]. |
