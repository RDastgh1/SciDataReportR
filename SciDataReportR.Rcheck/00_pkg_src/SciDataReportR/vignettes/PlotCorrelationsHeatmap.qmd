...
one per significant heatmap cell.
...
> These `plotSigCorrelations()` / `AssemblePlots()` chunks are shown but not
> executed when the vignette is built, because the underlying `ggstatsplot`
> scatterplots require graphics fonts that are not available on all systems.
> Run them interactively to reproduce the figures.
