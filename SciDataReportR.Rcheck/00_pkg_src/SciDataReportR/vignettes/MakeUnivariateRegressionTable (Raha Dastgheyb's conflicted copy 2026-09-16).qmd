...
`FormattedTable` is the report-facing wide table: each outcome has its own
column spanner with an estimate (95% CI) and p-value. Significant effects carry
stars and are bolded.
...
`LargeTable` uses the same wide outcome layout but keeps N, estimate, standard
error, confidence limits, and p-value as separate columns for QC.
