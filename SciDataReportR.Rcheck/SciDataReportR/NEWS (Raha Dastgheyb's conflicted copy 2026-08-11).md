...
* The `occupancy` bar chart is no longer attached to `ModelInfo$plots` by
  `CreateMclustClusterModel()`, `CreateKMeansClusterModel()`,
  `CreatePCAMclustClusterModel()`, `CreatePCAKMeansClusterModel()`,
  `CreateHDBSCANClusterModel()`, `CreateGowerPAMClusterModel()`,
  `CreateLatentClassClusterModel()`, `CreateMCAMclustClusterModel()`, or
...
* `PlotClusterOccupancy()` is unchanged and still exported, so the plot can
  still be drawn directly from a model's `df_with_clusters`.
...
* `Project*Cluster()` results gain `ProjectionFit`, which triages every projected
  ...
  `df_with_clusters`. This extends the triage `ProjectSOMCluster()` already
...
* Bootstrap stability resamples now preserve observed factor levels, and the MCA
  ...
  `ProjectMCAMclustCluster()` now warns and treats categories absent from the
