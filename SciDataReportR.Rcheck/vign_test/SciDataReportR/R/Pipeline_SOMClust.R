...
#' @param stability_resamples Number of participant bootstrap resamples used to
#'   assess reproducibility for every successful exploratory candidate. Defaults
#'   to \code{0} (disabled); use \code{50} for an exploratory stability screen.
#' @param stability_seed Integer seed for bootstrap resampling.
#' @param stability_progress Logical; if TRUE, print bootstrap progress messages.
#'
#' @details
...
#' reproducibility. Each bootstrap refits the same candidate SOM/LPA solution
...
#' information criteria, entropy, bootstrap reproducibility, and the combined
...
#'           (plus reproducibility when bootstrap stability is enabled)
...
#'     \item \code{ModelInfo_MClust$Stability}: bootstrap replicate, cluster
...
#'
#' # Helper: round the numeric columns and add a frozen header
...
#' review$ModelInfo_SOM$plots$Circular
#' review$ModelInfo_SOM$plots$Line
#' review$ModelInfo_SOM$SOMFit$plots$distance_by_cluster_box
#' review$ProbFit$plots$individual_MaxProbBoxplot
#'
...
#' }
#' @export
CreateSOMMclustClusterModel <- function(data,
    variables = NULL,
  ...
  if (method == "finalize" && stability_resamples > 0L) {
    stable_model <- Recall(
      ...
      final_k = NULL, final_model = NULL,
      ClusterName = ClusterName, ZScoreType = ZScoreType,
        ...
        for (replicate in seq_len(stability_resamples)) {
          if (stability_progress) {
            message(
              ...
              ", bootstrap ", replicate, "/", stability_resamples,
              ": model ", candidate_model, ", k = ", candidate_k
            )
          }

          set.seed(as.integer(stability_seed + candidate_index * 100000L + replicate))
          bootstrap_rows <- sample(reference_rows, length(reference_rows), replace = TRUE)
          df_bootstrap <- df[bootstrap_rows, , drop = FALSE]

          bootstrap_result <- tryCatch(
            CreateSOMMclustClusterModel(
              data = df_bootstrap,
              variables = vars_used,
              method = "finalize",
              ...
              ClusterName = ".scidr_stability_cluster",
          ...
          if (inherits(bootstrap_result, "error")) {
            replicate_rows[[length(replicate_rows) + 1L]] <- dplyr::tibble(
              ...
              Status = "failed",
              ARI = NA_real_,
              Error = conditionMessage(bootstrap_result)
            )
            next
          }

          projection_result <- tryCatch(
            suppressWarnings(ProjectSOMMclustCluster(
  ...
  mdata_cols <- intersect(
    c("AIC", "BIC", "Entropy", "ReproducibilityScore"),
    names(mdata_wide)
  )
  ...
  out$ModelInfo_Mclust <- out$ModelInfo_MClust
  ...
  out$CandidateAudit <- ModelInfo_MClust$diagnostics$lpa_fit_diagnostics
...
#' @rdname CreateSOMMclustClusterModel
#' @param ... Arguments passed to [CreateSOMMclustClusterModel()].
#' @export
Pipeline_SOMClust <- function(...) {
  CreateSOMMclustClusterModel(...)
}

#' @description Deprecated alias for [CreateSOMMclustClusterModel()].
#' @rdname CreateSOMMclustClusterModel
#' @export
