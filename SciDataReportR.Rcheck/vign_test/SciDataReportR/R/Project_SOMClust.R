...
#' )
#' projected <- ProjectSOMCluster(object = model, new_df = df_Projection)
...
#' projected$ProbFit$plots$individual_MaxProbBoxplot
#' }
#' @export
ProjectSOMMclustCluster <- function(
    object,
    new_df,
    ClusterName = NULL,
  ...
  # Standard projection contract; retain SOMProj's legacy diagnostics below.
  individual_tbl$Projection_Fit_Class[individual_tbl$Projection_Fit_Class == "Poor SOM fit"] <-
    "Poor fit to training structure"
  SOMProj$individual <- individual_tbl
  SOMProj$summary <- ProjectionDiagnostics
  SOMProj$by_cluster <- cluster_fit_summary
  SOMProj$out_of_support <- out_of_range_summary %>%
    dplyr::filter(!is.na(.data$n_out_of_range), .data$n_out_of_range > 0)
  SOMProj$policy <- list(
  ...
  if (all(c("SOM_X", "SOM_Y") %in% names(assignment_data))) {
    SOMProj$plots$som_grid_map <- PlotClusterMap(
...
#' @rdname ProjectSOMMclustCluster
#' @param ... Arguments passed to [ProjectSOMMclustCluster()].
#' @export
Project_SOMClust <- function(...) {
  ProjectSOMMclustCluster(...)
}

#' @description Deprecated alias for [ProjectSOMMclustCluster()].
#' @rdname ProjectSOMMclustCluster
#' @export
