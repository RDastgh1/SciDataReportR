...
test_that("categorical benchmark supports truth-known LCA and MCA examples", {
  ...
  model_LCA <- CreateLatentClassClusterModel(
    ...
    nrep = 5)
  projected_LCA <- ProjectLatentClassCluster(model_LCA, df_Projection)
  model_MCA <- CreateMCAMclustClusterModel(
    df_Training, vars_Categorical, method = "finalize", final_k = 4,
    final_model = "EEI")
  projected_MCA <- ProjectMCAMclustCluster(model_MCA, df_Projection)
