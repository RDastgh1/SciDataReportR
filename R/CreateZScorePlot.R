                              ...
                              Relabel = TRUE, sort = TRUE, RemoveXAxisLabels = TRUE, Ordinal = TRUE)
{
  ...
  melted <- tidyr::pivot_longer(scaledData, cols = Variables,
  ...
  GroupMeans$FDRpval <- stat.test$p.adj[match(GroupMeans$variable,
                                              stat.test$variable)]
  GroupMeans$Category <- ifelse(is.null(VariableCategories),
                                NA, VariableCategories[match(GroupMeans$variable, Variables)])
