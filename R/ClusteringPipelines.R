...
# evaluated by each public constructor so even an explicitly supplied default
# is caught rather than silently ignored.
.ValidateClusterLifecycle <- function(method, exploratory, finalized) {
  wrong <- if (identical(method, "finalize")) exploratory else finalized
  wrong <- names(wrong)[unlist(wrong)]
  if (length(wrong)) {
    expected <- if (identical(method, "finalize")) "exploratory" else "finalize"
    stop(
      "method = '", method, "' cannot be combined with ",
      paste(wrong, collapse = ", "), ". These settings belong to method = '",
      expected, "'.", call. = FALSE)
  }
}
...
.ClusterSpecification <- function(method, variables, seeds = list(),
  ...
  list(
    ...
    # RNGkind() records R's uniform, normal, and sampling generators.
...
#'   `ICL` are reported on the scale `mclust` returns them on, where higher is
#'   better; `AIC` is reported on the conventional scale, where lower is
#'   better. Entropy is higher-is-better classification separation and maximum
#'   uncertainty is lower-is-better. Shared stability fields are defined in the clustering
#'   reference vignette.
...
#'   `icl`, `entropy`, `centre_heatmap`, `centre_profile`, and `profiles`;
...
#'   distance reference and holds the projected distance histogram, the
...
#'   size) and subsample stability. WSS is lower-is-better; between-cluster
...
CreateClusterModel_KMeans <- function(data, variables = NULL,
  ...
  base$ProbFit <- .ClusterProbFit(
    ...
    "Distance margin to the second-nearest centroid", bounded = FALSE)
  if (!is.null(Stability)) Stability$plots <- .ClusterStabilityPlots(Stability)
  out <- c(list(method = method, vars_used = prep$variables, ClusterVariableName = ClusterVariableName,
                Preprocessing = list(ZScoreType = Scaling, Scaling = Scaling, ZScoreObject = prep$ZScoreObject),
...
#'   distance histogram, the frozen-centroid `separation_map`, the
#'   fit-class summaries and method-specific projection diagnostics.
#' @examples
#' \donttest{
#' data("SimulatedPhenotypeData")
...
.CreatePCACluster <- function(data, variables, algorithm, method, k_range,
  ...
  FitScoreModel <- function(fit_method, candidate_k = final_k,
    ...
    if (algorithm == "mclust") {
      if (identical(fit_method, "finalize")) {
        CreateClusterModel_MClust(
          df_Scores, score_vars, method = "finalize", final_k = candidate_k,
          final_model = candidate_model, ZScoreType = "None",
          ClusterVariableName = ClusterVariableName, seed = seed, stability_resamples = 0L)
      } else {
        CreateClusterModel_MClust(
          df_Scores, score_vars, method = "exploratory", k_range = k_range,
          models = models, ZScoreType = "None", ClusterVariableName = ClusterVariableName,
      ...
      if (identical(fit_method, "finalize")) {
        CreateClusterModel_KMeans(
          df_Scores, score_vars, method = "finalize", final_k = candidate_k,
          ZScoreType = "None", ClusterVariableName = ClusterVariableName,
...
#'   subsample stability. `ModelInfo$plots` leads with the reduction layer's
...
#' @param pca_variance_threshold Cumulative variance retained by the existing PCA workflow.
#' @inheritSection cluster-stability-output Stability output
#' @return A frozen PCA and K-means pipeline with full-pipeline subsample
...
# Distance from every training case to the nearest core point other than
# itself. This is the training-side counterpart of the nearest-core distance a
# projected case is scored on, so the two are on one scale and the frozen
# high-distance cutoff means the same thing in both.
.HDBSCANNearestCoreDistance <- function(X, core_clusters) {
  core_rows <- which(core_clusters > 0)
  if (!length(core_rows)) return(rep(NA_real_, nrow(X)))
  core_X <- X[core_rows, , drop = FALSE]
  if (nrow(core_X) < 2L) {
    distances <- sqrt(rowSums(sweep(X, 2, core_X[1, ], "-")^2))
    # The only core point has no other core point to be measured against.
    distances[core_rows] <- NA_real_
    return(distances)
  }
  neighbours <- dbscan::kNN(core_X, k = 2L, query = X)
  nearest_row <- core_rows[neighbours$id[, 1]]
  ifelse(nearest_row == seq_len(nrow(X)),
         neighbours$dist[, 2], neighbours$dist[, 1])
}
...
#'   subsample ARI/Jaccard and noise-recovery metrics. Persistence is
...
#'   holds the nearest-core-distance histogram and `outlier_score_by_cluster`,
#'   both measured against the same frozen reference a projected case is
#'   triaged on; `ProbFit$plots` holds
...
CreateClusterModel_HDBSCAN <- function(data, variables = NULL,
  ...
  ind <- dplyr::tibble(
    ...
    NearestCoreDistance = rep(NA_real_, nrow(prep$data)))
  ...
  ind$NearestCoreDistance[prep$complete_rows] <-
    .HDBSCANNearestCoreDistance(X, fit$cluster)
  ...
  base <- .ClusterOutput(prep, fit$cluster, ClusterVariableName, ind,
    ModelInfo, Stability = Stability)
  individual <- base$ProbFit$individual
  # The frozen reference has to be built on the quantity projection actually
  # measures. A training case's core distance is a k-nearest-neighbour density
  # estimate; a projected case's is its distance to the closest training core
  # point. Triaging the second against quantiles of the first compared two
  # different scales, so the reference is built on the training cases' own
  # distance to the nearest *other* core point instead.
  ModelInfo$FitDiagnostics <- .ClusterFitDiagnostics(
    individual, "NearestCoreDistance",
    "Distance to the nearest training core point")
...
#'   holds the projected distance histogram, the frozen-space
#'   `separation_map`, the fit-class summaries and the method-specific
#'   `nearest_core_support` plot.
#' @examples
#' \donttest{
#' data("SimulatedPhenotypeData")
#' df_Training <- dplyr::filter(SimulatedPhenotypeData, .data$Cohort == "Training")
...
# PAM fits on cluster::daisy(metric = "gower"), so every distance this pipeline
# reports afterwards has to be the same coefficient. daisy does not treat all
# non-numeric columns alike: ordered factors are rank-scaled and compared on
# that scale, and logicals are asymmetric binary, where a shared FALSE is an
# absence of evidence and drops out of both the numerator and the denominator.
# Scoring those columns by simple matching instead produced a second, different
# metric, under which a training case could sit nearer a medoid other than the
# one PAM assigned it to.
#
# The specification is frozen at fit time: projected cases are scored against
# the training ranges and levels, not against their own cohort's.
.GowerSpecification <- function(training, variables) {
  stats::setNames(lapply(variables, function(variable) {
    values <- training[[variable]]
    if (is.logical(values)) {
      list(type = "asymm")
    } else if (is.ordered(values)) {
      list(type = "ordinal", levels = levels(values))
    } else if (is.numeric(values)) {
      list(type = "interval", range = diff(range(values, na.rm = TRUE)))
    } else {
      list(type = "nominal", levels = levels(factor(values)))
    }
  }), variables)
}

# Map a column onto the numeric scale its Gower contribution is measured on.
.GowerEncode <- function(values, specification) {
  switch(specification$type,
    interval = as.numeric(values),
    # daisy replaces an ordinal variable by its rank and rescales to [0, 1],
    # then treats the result as interval.
    ordinal = {
      codes <- as.integer(factor(as.character(values), levels = specification$levels))
      if (length(specification$levels) > 1L) {
        (codes - 1) / (length(specification$levels) - 1)
      } else as.numeric(codes) * 0
    },
    asymm = as.numeric(as.logical(values)),
    nominal = as.integer(factor(as.character(values), levels = specification$levels)))
}

.GowerToMedoids <- function(data, medoids, variables, specification) {
  numerator <- matrix(0, nrow(data), nrow(medoids))
  denominator <- matrix(0, nrow(data), nrow(medoids))
  for (variable in variables) {
    this_spec <- specification[[variable]]
    x <- .GowerEncode(data[[variable]], this_spec)
    y <- .GowerEncode(medoids[[variable]], this_spec)
    contribution <- switch(this_spec$type,
      interval = if (is.na(this_spec$range) || this_spec$range == 0) {
        matrix(0, length(x), length(y))
      } else abs(outer(x, y, "-")) / this_spec$range,
      ordinal = abs(outer(x, y, "-")),
      outer(x, y, function(a, b) as.numeric(a != b)))
    weight <- outer(!is.na(x), !is.na(y), "&")
    if (identical(this_spec$type, "asymm")) {
      # `%in%` rather than `==` so a missing value never produces an NA weight.
      weight <- weight & !outer(x %in% 0, y %in% 0, "&")
    }
...
CreateClusterModel_Gower_PAM <- function(data, variables = NULL,
  ...
  # One dissimilarity for every candidate k, for the training map, and as the
  # reference the frozen specification below has to reproduce.
  training_dissimilarity <- cluster::daisy(training, metric = "gower")
  fits <- lapply(ks, function(k) cluster::pam(training_dissimilarity, k = k, diss = TRUE))
  ...
  gower_specification <- .GowerSpecification(training, variables)
  ...
  d <- .GowerToMedoids(training, medoids, variables, gower_specification)
  ...
  gower_coordinates <- stats::cmdscale(training_dissimilarity, k = 2L)
  ...
  ModelInfo <- list(
    ...
    levels = lapply(training, levels),
    GowerSpecification = gower_specification,
    GowerCoordinates = gower_assignment,
  ...
  base$ProbFit <- .ClusterProbFit(
    individual, "AssignmentMargin",
    "Gower distance margin to the second-nearest medoid", bounded = FALSE)
  if (!is.null(Stability)) Stability$plots <- .ClusterStabilityPlots(Stability)
...
#'   distance histogram, a `medoid_map` placing cases by their two nearest
#'   medoid distances, fit-class summaries, and the fit-class bar.
#' @examples
#' \donttest{
#' data("SimulatedPhenotypeData")
#' df_Training <- dplyr::filter(SimulatedPhenotypeData, .data$Cohort == "Training")
...
ProjectCluster.Pipeline_Gower_PAM <- function(object, new_df, ClusterVariableName = object$ClusterVariableName, ...) {
  ...
  out_of_support <- dplyr::bind_rows(lapply(object$vars_used, function(variable) {
    ...
    # they are the ones a projected cohort can fall outside of.
    this_spec <- specification[[variable]]
    if (!this_spec$type %in% c("nominal", "ordinal")) return(dplyr::tibble())
    known <- this_spec$levels
    if (is.null(known)) return(dplyr::tibble())
  ...
  d <- .GowerToMedoids(
    prep$data[prep$complete_rows, object$vars_used, drop = FALSE],
    object$ModelInfo$medoids, object$vars_used, specification)
