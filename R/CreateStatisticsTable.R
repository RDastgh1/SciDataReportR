
#' Create Statistics Table
#'
#' Generate a table of statistics including means, standard deviations, counts, and p-values.
#'
#' @param data The data frame containing the variables of interest.
#' @param TargetVar The target variable for which statistics will be calculated.
#' @param alternative Hypothesis alternative for two-group tests. `"greater"`
#' tests whether the second target-factor level is greater than the first.
#' @return A formatted HTML table displaying statistics.
#' @importFrom dplyr mutate rename
#' @param Data \strong{Deprecated} (since 19.15.0). Use \code{data} instead.
#' @examples
#' \donttest{
#' data(SampleData)
#' data(SampleVariableTypes)
#'
#' df_Labelled <- RevalueData(SampleData, SampleVariableTypes)$RevaluedData
#' df_Statistics <- df_Labelled[, c("Diagnosis", "age", "AXL", "Ab_42", "p_tau")]
#'
#' # Group means (SD), missing counts, and a p-value for every variable
#' statistics_table <- CreateStatisticsTable(
#'   data = df_Statistics,
#'   TargetVar = "Diagnosis"
#' )
#'
#' # The returned kableExtra table prints directly in Quarto, R Markdown, and
#' # the reference page. Do not wrap it in `browsable()` a second time.
#' statistics_table
#' }
#' @export

CreateStatisticsTable <- function(data,
    TargetVar,
    alternative = c("two.sided", "greater", "less"),
    Data = lifecycle::deprecated()) {
  # Deprecated argument shims (SciDataReportR 19.15.0)
  if (lifecycle::is_present(Data)) {
    lifecycle::deprecate_warn("19.15.0", "CreateStatisticsTable(Data)", "CreateStatisticsTable(data)")
    data <- Data
  }
  if (!missing(data)) Data <- data
  alternative <- match.arg(alternative)

  for (pkg in c("arsenal", "data.table", "kableExtra")) {
    if (!requireNamespace(pkg, quietly = TRUE)) {
      stop(
        "Package '", pkg, "' is required by CreateStatisticsTable(). ",
        "Install it with install.packages('", pkg, "')."
      )
    }
  }

  tab1 <- arsenal::tableby(as.formula(paste(TargetVar, "~ .")), data = Data, digits.pct = 1, digits.count = 1, numeric.stats = c("Nmiss2", "meansd"), cat.test = "chisq")
  sd <- as.data.frame(summary(tab1, text = FALSE))
  colnames(sd)[length(colnames(sd))] <- "p_value"
  colnames(sd)[1] <- "Variable"

  vars_Statistics <- setdiff(names(Data), TargetVar)
  comparison_table <- MakeComparisonTable(
    data = Data,
    group_var = TargetVar,
    variables = vars_Statistics,
    alternative = alternative,
    suppress_warnings = TRUE
  )
  comparison_p <- comparison_table$table_body %>%
    dplyr::filter(.data$row_type == "label") %>%
    dplyr::select("variable", "p.value")
  sd$p_value <- comparison_p$p.value[match(sd$Variable, comparison_p$variable)]
  pvals <- sd$p_value
  pvals[is.na(pvals)] <- 1


  old_opts <- options(kableExtra.auto_format = TRUE)
  on.exit(options(old_opts), add = TRUE)

  sd <- data.table::as.data.table(sd)
  StatTable <- sd %>% dplyr::mutate(p_value = kableExtra::cell_spec(p_value, "html", background = ifelse(pvals < 0.05, "yellow", ""))) %>% dplyr::rename("p-value" = p_value) %>%
    kableExtra:: kable(format = "html", escape = F)%>% kableExtra::kable_styling(bootstrap_options = c("striped", "hover", "condensed", "responsive"))%>%
    kableExtra::scroll_box(width = "100%", height = "500px")
  return(StatTable)
}
