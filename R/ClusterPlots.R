...
# `bounded` says whether the diagnostic is a probability. Only a probability may
# be pinned to a 0-1 axis: the distance margins and nearest-core distances that
# also arrive here are unbounded, and fixing the axis silently discarded almost
# every point instead of showing the distribution.
.ClusterConfidencePlots <- function(individual, value, value_label,
  ...
  if (isTRUE(bounded)) {
    p_density <- p_density + ggplot2::scale_x_continuous(
...
.ClusterStabilityPlots <- function(Stability) {
  ...
  if (!is.null(Stability$cluster_recovery) &&
    ...
    plots$cluster_recovery <- ggplot2::ggplot(
      ...
      ggplot2::theme_bw() +
      ggplot2::labs(
        title = "Per-cluster subsample recovery",
        x = "Cluster", y = "Jaccard")
  }
    ...
    if (nrow(df_Metrics)) {
      plots$partition_metrics <- ggplot2::ggplot(
        ...
        ggplot2::theme_bw() + ggplot2::labs(
          title = "Subsample partition agreement metrics", x = NULL, y = "Refits")
    }
  }
  if (!is.null(Stability$cluster_inclusion) && nrow(Stability$cluster_inclusion)) {
    plots$cluster_inclusion <- ggplot2::ggplot(
      ...
      ggplot2::labs(title = "Cluster inclusion across subsample refits",
  ...
  if (!is.null(Stability$coassignment) &&
    ...
    if (!is.null(matrix)) {
      ...
      plots$coassignment <- ggplot2::ggplot(df_Matrix,
        ...
        ggplot2::labs(title = "Subsample co-assignment probability", x = "Participant", y = "Participant")
    }
  }
  plots
}
