...
CreateSummaryTable <- function(Data, Variables = NULL, numdecimals = 2, Relabel = TRUE, Ordinal = FALSE, ScrollBoxHeight = "700px") {
    ...
    suppressWarnings({
      ...
      SummaryTable <- d2 %>% rownames_to_column("Variable") %>%
        mutate(Skewness = cell_spec(round(Skewness, 2), "html",
                                    background = if_else(abs(Skewness) > 10, "yellow",
                                                         "", missing = "grey")), Kurtosis = cell_spec(round(Kurtosis,
                                                                                                            2), "html", background = if_else(abs(Kurtosis) >
                                                                                                                                               10, "yellow", "", missing = "grey")), IQR = cell_spec(round(IQR,
                                                                                                                                                                                                           2), "html", background = if_else(abs((IQR/Std.Dev/1.34) -
                                                                                                                                                                                                                                                  1) > 0.5, "yellow", "", missing = "grey")), Pct.Valid = cell_spec(round(Pct.Valid,
                                                                                                                                                                                                                                                                                                                          2), "html", background = if_else(Pct.Valid <
                                                                                                                                                                                                                                                                                                                                                             70, "red", "", missing = "grey"))) %>% kable(format = "html",
                                                                                                                                                                                                                                                                                                                                                                                                          escape = FALSE, digits = 2, row.names = TRUE, caption = "Descriptive Summary Table. IQR, Skewness, and Kurtosis are highlighted in yellow if they are indicative of a non-normal distribution. Pct.Valid is highlighted in red if over 30% of data is missing") %>%
