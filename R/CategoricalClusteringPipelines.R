...
#'   uncertainty histogram, the `posterior_map`, the fit-class summaries and
#'   method-specific projection diagnostics.
#' @examples
#' \donttest{
#' data("SimulatedPhenotypeData")
#' df_Training <- dplyr::filter(SimulatedPhenotypeData, .data$Cohort == "Training")
...
CreateClusterModel_MCA_MClust <- function(data, variables,
  ...
  review_model <- if (identical(method, "finalize")) {
    CreateClusterModel_MClust(
      scores, score_vars, method = "finalize", final_k = final_k,
      final_model = final_model, ZScoreType = "None",
      ClusterVariableName = ClusterVariableName, seed = seed,
      stability_resamples = 0L)
  } else {
    CreateClusterModel_MClust(
      scores, score_vars, method = "exploratory", k_range = k_range,
  ...
  if (stability_resamples > 0L) {
    ...
    stabilities <- lapply(seq_len(nrow(candidate_rows)), function(i) {
      ...
      reference_fit <- CreateClusterModel_MCA_MClust(
        data, variables, method = "finalize", final_k = candidate_k,
