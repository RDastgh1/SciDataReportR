...
.BuildHeatmapCaption <- function(score_type,
  ...
  star_text <- switch(
    star_p,
    raw = "Stars show raw p-values (* p < 0.05, ** p < 0.01, *** p < 0.001).",
    adjusted = "Stars show adjusted p-values (* p < 0.05, ** p < 0.01, *** p < 0.001).",
...
.ComputePairwiseReferentContrasts <- function(data,
  ...
  results <- purrr::map_dfr(seq_len(nrow(variable_map)), function(i) {
    ...
    template <- tibble::tibble(
      ...
      NReferent = sum(df_cc[[group_var]] == Referent, na.rm = TRUE),
