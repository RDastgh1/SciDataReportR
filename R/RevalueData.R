...
RevalueData <- function(DatatoRevalue, VarTypes, missingVal = -999, splitchar = ";") {
  ...
  # Normalize smart quotes -> ASCII, coerce to character, trim
  norm_cols <- intersect(c("Variable","Recode","Code","Type","Label","MissingCode","Missing"),
                         names(VarTypes))
  for (cc in norm_cols) {
  ...
  if (!"Variable" %in% names(VarTypes)) stop("VarTypes must have a 'Variable' column.")
  ...
  # Back-compat MissingCode <- Missing if absent/blank
  ...
  # If duplicates of Variable exist, collapse rows by Variable (no helper fn)
  ...
  # --- Pick variables present in data --------------------------------------
  ...
  for (i in seq_along(vars)) {
    ...
    # ---- MissingCode to NA (supports multiple tokens: , ; |) --------------
    ...
    # ---- Recoding & labeling ----------------------------------------------
    rc      <- tolower(trimws(vt_in$Recode[idx]))
    do_reco <- isTRUE(rc %in% c("yes","y","1","true","t"))
    ...
    if (do_reco) {
      ...
      if (is.na(codestr) || !nzchar(codestr)) {
        ...
        # Parse Code map without helpers: split pairs, then split by =, =>, or :
        parts <- strsplit(codestr, splitchar, fixed = TRUE)[[1]]
        parts <- parts[nzchar(trimws(parts))]
        if (length(parts) == 0L) {
          warninglist <- c(warninglist, paste0(var, ": Code string has no pairs."))
        } else {
          ...
          for (p in parts) {
            ...
            if (length(ab) >= 2 && nzchar(ab[1]) && nzchar(ab[2])) {
              from <- c(from, ab[1]); to <- c(to, ab[2])
            }
          }
          if (!length(from)) {
            warninglist <- c(warninglist, paste0(var, ": Code parsing produced no valid key/value pairs."))
          } else {
            # Attach labels; storage depends on Type
            nameList <- to; names(nameList) <- from
            tmp <- sjlabelled::set_labels(RevaluedData[[var]],
                                          labels = nameList,
                                          force.labels = TRUE,
                                          force.values = TRUE)
            ...
            if (vartype %in% c("categorical","factor","ordinal","ordered factor","ordered")) {
              ...
              # Unknown/unspecified Type: keep original storage but keep labels
