...
add_r_and_stars <- function(res,
  ...
  # --- validate & locate plots inside `res` ---------------------------------
  ...
  plot_has_star_mapping <- function(p) {
    ...
    for (ly in p$layers) {
      if (is_text_layer(ly)) {
        ...
        if (!is.na(lab) && grepl("^stars", lab, ignore.case = TRUE)) return(TRUE)
      }
    }
    FALSE
  }
  ...
  pick_plot <- function() {
    ...
    stop("Couldn't find a valid ggplot in `res$FDRCorrected$plot` or `res$Unadjusted$plot`.")
  }
  ...
  # --- detect any original stars mapping in the chosen plot -----------------
  existing_star_col <- NA_character_
  for (ly in plot_obj$layers) {
    if (!is_text_layer(ly)) next
    lab <- safe_as_label(ly$mapping$label)
    if (!is.na(lab) && grepl("^stars", lab, ignore.case = TRUE) && lab %in% names(d)) {
      existing_star_col <- lab; break
    }
  }

  # --- helpers to build labels ----------------------------------------------
  stars_from_p <- function(pvec) {
    if (is.null(pvec)) return(rep(NA_character_, nrow(d)))
  ...
  pick_stars <- function() {
    ...
    if (star_from == "column") {
      if (is.null(star_col) || !star_col %in% cols) {
        stop("star_from='column' requires a valid `star_col` present in the plot data.")
      }
  ...
  # --- build new label columns ----------------------------------------------
  fmt <- paste0("%.", r_digits, "f")
  d$label_r    <- sprintf(fmt, d[[r_var]])
  d$label_star <- pick_stars()

  # --- optionally drop original star text layers before redrawing -----------
  if (remove_existing_stars && any(!is.na(d$label_star))) {
    if (length(plot_obj$layers)) {
      keep <- vapply(plot_obj$layers, function(ly) {
        if (!is_text_layer(ly)) return(TRUE)
        lab <- safe_as_label(ly$mapping$label)
        is.na(lab) || !grepl("^stars", lab, ignore.case = TRUE)
      }, logical(1))
      plot_obj$layers <- plot_obj$layers[keep]
    }
  }
